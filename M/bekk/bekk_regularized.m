function [parameters, ll, Ht, output, VCV, scores, DiagnosticsDiagonal, DiagnosticsLDiagonal, DiagnosticsFull, DiagnosticsLFull] = bekk_regularized(data,dataAsym,p,o,q,type,startingVals,options,lambdasDiagonal,matricesDiagonal,regularizationDiagonal,lambdasFull,matricesFull,regularizationFull,rT0,lossFunction)
...
if type>=2
    ...
    [parameters,ll,H,output]=bekk(data_rest,dataAsym,p,o,q,1,[],options);
    [C,A,~,B]=bekk_parameter_transform(parameters,p,o,q,k,1);
    rstartingVals=[parameters(1:((k^2-k)/2+k)); diag(A); diag(B)];
    H_rf=bekk_forecast(data,H,parameters,p,o,q,1);
    
    for i=1:Nrow
        strcat("Diagonal lambda=",num2str(lambdasDiagonal(i)))
        if ~isinf(lambdasDiagonal(i))
            [parameters,ll,H,output]=bekk(data_rest,dataAsym,p,o,q,2,rstartingVals,options,DiagnosticsDiagonal.Lambda(i),matricesDiagonal,regularizationDiagonal,0,matricesFull,regularizationFull);
            [C,A,~,B]=bekk_parameter_transform(parameters,p,o,q,k,2);
            rstartingVals=[parameters(1:((k^2-k)/2+k)); diag(A); diag(B)];
            H_rf=bekk_forecast(data,H,parameters,p,o,q,2);
        end
        ...
        DiagnosticsDiagonal.Penalty(i)=bekk_penalty(C,A,B,1,matricesDiagonal,regularizationDiagonal,0,matricesFull,regularizationFull);
    ...
    error('for regularized BEKK, type must be either 2 (diagonal BEKK) or 3 (full BEKK)')
end
...
if type>=3
    ...
    [parameters,ll,H,output]=bekk(data_rest,dataAsym,p,o,q,2,rstartingVals,options,lambdaDiagonal,matricesDiagonal,regularizationDiagonal,0,matricesFull,regularizationFull);
%     [parameters,ll,H,output]=caw(data_rest,dataAsym,p,o,q,2,rstartingVals,options,0,matricesDiagonal,regularizationDiagonal,0,matricesFull,regularizationFull);
    [C,A,~,B]=bekk_parameter_transform(parameters,p,o,q,k,2);
    rstartingVals=[parameters(1:((k^2-k)/2+k)); A(:); B(:)];
    H_rf=bekk_forecast(data,H,parameters,p,o,q,2);
    
    for i=1:Nrow
        strcat("Full lambda=",num2str(lambdasFull(i)))
        if ~isinf(lambdasFull(i))
            [parameters,ll,H,output]=bekk(data_rest,dataAsym,p,o,q,3,rstartingVals,options,lambdaDiagonal,matricesDiagonal,regularizationDiagonal,DiagnosticsFull.Lambda(i),matricesFull,regularizationFull);
%             [parameters,ll,H,output]=caw(data_rest,dataAsym,p,o,q,3,rstartingVals,options,0,matricesDiagonal,regularizationDiagonal,DiagnosticsFull.Lambda(i),matricesFull,regularizationFull);
            [C,A,~,B]=bekk_parameter_transform(parameters,p,o,q,k,3);
            rstartingVals=[parameters(1:((k^2-k)/2+k)); A(:); B(:)];
            H_rf=bekk_forecast(data,H,parameters,p,o,q,3);
        end
        ...
        DiagnosticsFull.Penalty(i)=bekk_penalty(C,A,B,0,matricesDiagonal,regularizationDiagonal,1,matricesFull,regularizationFull);
...
[parameters, ll, Ht, output, VCV, scores] = bekk(data,dataAsym,p,o,q,type,startingVals,options,lambdaDiagonal,matricesDiagonal,regularizationDiagonal,lambdaFull,matricesFull,regularizationFull);
